...
5. **Solution membership.** Confirm where the Avalonia head is built from in CI. Do not add `Gum.Avalonia` to `GumFull.sln` if that forces WPF/WinForms targets onto the cross-platform build (Phase 1 risk). Prefer `AllLibraries.sln` or a dedicated cross-platform solution; record the decision.
6. **Reserve test slots for Phase 22.** Add commented placeholders in the test allow-list for the migration's headless Avalonia test projects, following the Bucket A/B/C policy comments already in `build-and-test.yaml`, so Phase 22 only has to fill them in.
7. **Verify no WPF-pipeline regression.** Confirm `Gum.sln` / `GumFull.sln` build steps, the headless test allow-list, and the Windows zip release are unchanged in behavior and timing characteristics.
...
- **Best done alongside / right after Phase 5:** the `AddGumCore`/`AddGumWpf` split is exactly what the purity guard protects; running the guard as Phase 5 lands catches `System.Windows` leaks into core immediately. Ideally the guard is stood up *concurrently* with Phase 5 (against the Phase 1 scaffolding), not strictly after, so the `AddGumCore`/`AddGumWpf` split is validated as it lands rather than retroactively.
...
- **Guard false positives.** A purity guard that's too broad can fail on legitimate transitive references (e.g. a multi-targeted neutral library). Tune it to flag only `UseWPF`/`UseWindowsForms`/`*-windows` in the Avalonia closure, and test it against the Phase 1 scaffolding before relying on it.
