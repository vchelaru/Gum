...
- **Portable ViewModel base** — [../../Gum/Mvvm/ViewModel.cs](../../Gum/Mvvm/ViewModel.cs) is a home-grown base (dictionary-backed `Get`/`Set` + `[DependsOn]`) depending only on `INotifyPropertyChanged` / `INotifyCollectionChanged`, not `DependencyObject`. (`CommunityToolkit.Mvvm` is used for `RelayCommand`/messaging, but the VMs do not derive from `ObservableObject`.) **Caveat:** the base is portable, but ~20 concrete VMs still expose WPF types on their public surface (`System.Windows.Visibility`, `ICommand`, `BitmapImage`, `ICollectionView`, `DispatcherTimer`, `Microsoft.Xaml.Behaviors`) — so they are **not** a clean "move unchanged"; a VM-layer de-WPF sweep is required (owned by Phase 5, guarded by Phase 7).
...
**Spike the canvas first — now its own phase (12).** The canvas-hosting approach (render KNI to a `WriteableBitmap` vs `NativeControlHost`) is the make-or-break risk, so the throwaway de-risking spike is broken out as Phase 12: it runs standalone, in parallel with (ideally ahead of) Phases 5/10, depends on none of them, and gates the Phase 15 integration. A no-go in Phase 12 forces a strategy change before further investment. The favorable finding (`GraphicsDeviceControl` already renders to a Bgra32 `RenderTarget2D` and reads it back) is **true only for the current Windows DirectX 11 backend** (`nkast.Kni.Platform.WinForms.DX11`). **The real cross-platform work is greenfield: the editor has no KNI desktop-GL backend, so Phase 12 must first stand up a GL backend (`nkast.Kni.Platform.*` GL/SDL) and create a `GraphicsDevice` on macOS/Linux at all, then run the latency gate on *that* backend — not prove the WriteableBitmap present step on Windows DX11.** If desktop-GL headless device creation can't hit interactive latency, the canvas may ship Windows-only behind a flag and the cutover bar is renegotiated — this is the genuine make-or-break. Phase 15 then integrates the chosen approach behind the Phase 5 seam inside the Phase 10 shell.
...
- **Reuse mostly as-is:** [../../Gum/Mvvm/ViewModel.cs](../../Gum/Mvvm/ViewModel.cs) (the VM base), [../../Tools/Gum.ProjectServices/](../../Tools/Gum.ProjectServices/), all `I*` service interfaces, and the `CommunityToolkit.Mvvm` command/messaging layer.
- **De-WPF sweep (not "reuse as-is"):** the ~20 concrete ViewModels that expose WPF types on public members must be cleaned (e.g. `System.Windows.Visibility` → `bool` + converter, drop `BitmapImage`/`ICollectionView`/`DispatcherTimer`/`Microsoft.Xaml.Behaviors`). Owned by Phase 5; a Phase 7 guard scans VM assemblies (not just the DI graph) for `System.Windows.*` on public surfaces.
