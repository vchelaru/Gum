...
Gum creates the `Tooltip` for you when you assign `ToolTip`, so unlike other controls there is no instance in a screen through which you pick its visual. When you load a project from file, Gum looks for a component with the `TooltipBehavior` and uses the first one it finds as the visual for every tooltip. A `TextInstance` child in that component receives the tooltip's text.
