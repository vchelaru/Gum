...
/// <summary>
/// Template-styled ScrollViewer visual. Same shell pattern as
/// <see cref="ListBoxVisual"/> — 1 px-bordered rounded rectangle (CornerRadius=2,
/// built via <see cref="TemplateShapes"/>) with an outer Accent focus ring — and
/// the internal scroll/clip container inset 1 px on every side so the Template
/// ScrollBars don't sit flush against the border. The Forms-side
/// <c>new ScrollBar()</c> calls in the V3 base ctor resolve through the Template
...
public class ScrollViewerVisual : BaseScrollViewerVisual
{
        ...
        : base(fullInstantiation, tryCreateFormsObject)
    {
        ...
        // Template's ScrollBarVisual bakes its own thumb insets, so the bar
