...
public class WpfGraphicsDeviceControl : Grid, IDisposable, IRenderTargetFrameClient, ICanvasHost
{
    ...
    private void HandleRenderFrame()
    {
        ...
        // ActualWidth/Height are WPF device-independent units; the shared frame loop sizes the
        // render target and viewport directly from these, so they must be physical pixels here or
        // the canvas renders at the DIU resolution instead of the display's real one (#4681).
        int width = ToPhysicalPixelSize(ActualWidth, DpiScale);
        int height = ToPhysicalPixelSize(ActualHeight, DpiScale);
