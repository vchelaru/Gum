...
- [x] Every plugin project that has no canvas is `net10.0`. ConvertToJson, EventOutput (phase 40), PerformanceMeasurement (here), HtmlToGum (shared plugin in `Gum.Presentation`, 2026-09-11), GumForms, ImportFromGumx and SvgPlugin (phase 70); State Animation and CodeOutput are shared bodies plus a WPF view head each that retires at cutover. The Fonts and menu-refresh plugins moved into `Gum.Presentation` on 2026-09-11, so the Avalonia head has the Content menu's font cache entries and live Undo/Redo and Remove menu states.
