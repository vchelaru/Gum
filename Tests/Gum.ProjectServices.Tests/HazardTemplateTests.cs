...
/// <summary>
/// Validates the bundled Hazard theme template (#4135, the second theme ported to Add Forms
/// tool content after Bubblegum). Mirrors <see cref="BubblegumTemplateTests"/> - see that class
/// for the reasoning behind each check.
/// </summary>
public class HazardTemplateTests
{
    ...
    private static GumProjectSave LoadHazard()
    {
        StandardElementsManager.Self.Initialize();

        string hazardDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "Hazard");
        ProjectLoadResult result = new ProjectLoader().Load(Path.Combine(hazardDir, "GumProject.gumx"));
    ...
    public void Load_SharedBehaviorReferences_ShouldResolveWithNoSourceFileMissing()
    {
        ...
        GumProjectSave hazard = LoadHazard();
        GumProjectSave formsTemplate = LoadFormsTemplate();

        hazard.Behaviors.ShouldNotBeEmpty();
        formsTemplate.Behaviors.ShouldNotBeEmpty();
        hazard.Behaviors.ShouldAllBe(b => !b.IsSourceFileMissing, customMessage:
            "every Hazard behavior reference (shared or theme-local) must resolve to a real file");
    ...
    public void Load_SharedButtonBehavior_ShouldUsePerThemeDefaultImplementationOverride()
    {
        ...
        hazard.Behaviors.Single(b => b.Name == "ButtonBehavior").DefaultImplementation
            .ShouldBe("Hazard/Controls/Button");
    ...
    public void Load_ShouldNotReferenceRetiredStandardFiles()
    {
        string standardsDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "Hazard", "Standards");
    ...
    {
        // The whole point of Components/Hazard/Styles.gucx is that every control routes its
        ...
        {
            if (component.Name == "Hazard/Styles")
            {
                continue;
            }
    ...
    [Fact]
    public void AllPhysicalComponentFiles_ShouldBeHazardNamespaced()
    {
        ...
        // was left unrenamed during the Hazard/ namespacing pass still gets imported under
        ...
        List<string> componentFiles = Directory
            .GetFiles(Path.Combine(hazardDir, "Components"), "*.gucx", SearchOption.AllDirectories)
            .ToList();
        ...
        foreach (string file in componentFiles)
        {
            ...
            if (name != null && !name.StartsWith("Hazard/", StringComparison.Ordinal))
            {
    ...
    public void AllBehaviorFiles_ShouldBeSharedAndUnnamespaced()
    {
        ...
        // folder is normally empty. This test guards that a future Hazard-only behavior
        // (if one is ever added here instead of shared) stays unnamespaced.
        string behaviorsDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "Hazard", "Behaviors");
    ...
    [Fact]
    public void StylesPalette_ShouldExactlyMatchCodeOnlyHazardColors()
    {
        // Components/Hazard/Styles.gucx exists so tool content can reference one set of
        // color/text tokens instead of hardcoding values. Its swatch names must match
        // Themes/Gum.Themes.Hazard.MonoGame/HazardStyling.cs's HazardColors/HazardText
        // properties exactly, and carry no swatch beyond what that class defines - otherwise
        // the tool content silently drifts from the theme it's supposed to mirror. Excluded by
        // design (computed/alias properties with no distinct settable value of their own):
        // HoverFill (pure alias of Surface2), TextPrimary/TextMuted/Primary (guardrail aliases).
        string[] expectedColorSwatches =
        {
            "Accent", "AccentHover", "AccentPressed", "Background", "Band", "Border",
            "BorderHover", "DisabledAccent", "DisabledBorder", "DisabledFill", "DisabledText",
            "Ink", "Muted", "Placeholder", "PressedFill", "PressedText", "Selection",
            "Surface1", "Surface2", "Text", "TextBright"
        };
        string[] expectedTextStyles = { "Normal", "Strong" };

        GumProjectSave project = LoadHazard();
        ComponentSave styles = project.Components.Single(c => c.Name == "Hazard/Styles");
