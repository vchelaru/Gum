...
/// <summary>
/// Validates the bundled Dark Pro theme template. Mirrors <see cref="BubblegumTemplateTests"/> -
/// see that class for the reasoning behind each check.
/// </summary>
public class DarkProTemplateTests
{
    ...
    private static GumProjectSave LoadDarkPro()
    {
        StandardElementsManager.Self.Initialize();

        string darkProDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "DarkPro");
        ProjectLoadResult result = new ProjectLoader().Load(Path.Combine(darkProDir, "GumProject.gumx"));
    ...
    public void Load_SharedBehaviorReferences_ShouldResolveWithNoSourceFileMissing()
    {
        ...
        GumProjectSave darkPro = LoadDarkPro();
        GumProjectSave formsTemplate = LoadFormsTemplate();

        darkPro.Behaviors.ShouldNotBeEmpty();
        formsTemplate.Behaviors.ShouldNotBeEmpty();
        darkPro.Behaviors.ShouldAllBe(b => !b.IsSourceFileMissing, customMessage:
            "every DarkPro behavior reference (shared or theme-local) must resolve to a real file");
    ...
    public void Load_SharedButtonBehavior_ShouldUsePerThemeDefaultImplementationOverride()
    {
        ...
        darkPro.Behaviors.Single(b => b.Name == "ButtonBehavior").DefaultImplementation
            .ShouldBe("DarkPro/Controls/Button");
    ...
    public void Load_CircleInstances_ShouldNotCarryRadiusVariable()
    {
        ...
        GumProjectSave project = LoadDarkPro();
    ...
    public void Load_ShouldNotReferenceRetiredStandardFiles()
    {
        string standardsDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "DarkPro", "Standards");
    ...
    {
        // The whole point of Components/DarkPro/Styles.gucx is that every control routes its
        ...
        {
            if (component.Name == "DarkPro/Styles")
            {
                continue;
            }
    ...
    [Fact]
    public void AllPhysicalComponentFiles_ShouldBeDarkProNamespaced()
    {
        ...
        // was left unrenamed during the DarkPro/ namespacing pass still gets imported under
        ...
        List<string> componentFiles = Directory
            .GetFiles(Path.Combine(darkProDir, "Components"), "*.gucx", SearchOption.AllDirectories)
            .ToList();
        ...
        foreach (string file in componentFiles)
        {
            ...
            if (name != null && !name.StartsWith("DarkPro/", StringComparison.Ordinal))
            {
    ...
    public void AllBehaviorFiles_ShouldBeSharedAndUnnamespaced()
    {
        ...
        // folder is normally empty. This test guards that a future DarkPro-only behavior
        // (if one is ever added here instead of shared) stays unnamespaced.
        string behaviorsDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "DarkPro", "Behaviors");
    ...
    [Fact]
    public void StylesPalette_ShouldExactlyMatchCodeOnlyDarkProColors()
    {
        // Components/DarkPro/Styles.gucx exists so tool content can reference one set of
        // color/text tokens instead of hardcoding values. Its swatch names must match
        // Themes/Gum.Themes.DarkPro.MonoGame/DarkProStyling.cs's DarkProColors/DarkProText
        ...
        // TextPrimary/TextMuted/Primary (guardrail aliases).
        string[] expectedColorSwatches =
        {
            "Accent", "AccentDark", "AccentPressed", "Background", "Border", "BorderHover",
            "DisabledBorder", "DisabledFill", "DisabledText", "DisabledThumb", "HoverAccent",
            "Muted", "Placeholder", "PressedFill", "PressedText", "Surface1", "Surface2", "Text"
        };
        string[] expectedTextStyles = { "Normal", "Strong" };

        GumProjectSave project = LoadDarkPro();
        ComponentSave styles = project.Components.Single(c => c.Name == "DarkPro/Styles");
