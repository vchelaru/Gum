...
/// <summary>
/// Validates the bundled Meadow theme template. Mirrors <see cref="NeonTemplateTests"/> -
/// see that class for the reasoning behind each check.
/// </summary>
public class MeadowTemplateTests
{
    ...
    private static GumProjectSave LoadMeadow()
    {
        StandardElementsManager.Self.Initialize();

        string meadowDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "Meadow");
        ProjectLoadResult result = new ProjectLoader().Load(Path.Combine(meadowDir, "GumProject.gumx"));
    ...
    public void Load_SharedBehaviorReferences_ShouldResolveWithNoSourceFileMissing()
    {
        ...
        GumProjectSave meadow = LoadMeadow();
        GumProjectSave formsTemplate = LoadFormsTemplate();

        meadow.Behaviors.ShouldNotBeEmpty();
        formsTemplate.Behaviors.ShouldNotBeEmpty();
        meadow.Behaviors.ShouldAllBe(b => !b.IsSourceFileMissing, customMessage:
            "every Meadow behavior reference (shared or theme-local) must resolve to a real file");
    ...
    public void Load_SharedButtonBehavior_ShouldUsePerThemeDefaultImplementationOverride()
    {
        ...
        meadow.Behaviors.Single(b => b.Name == "ButtonBehavior").DefaultImplementation
            .ShouldBe("Meadow/Controls/Button");
    ...
    public void Load_CircleInstances_ShouldNotCarryRadiusVariable()
    {
        ...
        GumProjectSave project = LoadMeadow();
    ...
    public void Load_ShouldNotReferenceRetiredStandardFiles()
    {
        string standardsDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "Meadow", "Standards");
    ...
    {
        // The whole point of Components/Meadow/Styles.gucx is that every control routes its
        ...
        {
            if (component.Name == "Meadow/Styles")
            {
                continue;
            }
            ...
            // matching the code-only ColorPickerVisual convention other themes follow - Meadow
            // does not restyle ColorPicker in its code-only theme either.
            if (component.Name == "Meadow/Controls/ColorPicker")
            {
                continue;
            }
    ...
    [Fact]
    public void AllPhysicalComponentFiles_ShouldBeMeadowNamespaced()
    {
        ...
        // was left unrenamed during the Meadow/ namespacing pass still gets imported under
        ...
        List<string> componentFiles = Directory
            .GetFiles(Path.Combine(meadowDir, "Components"), "*.gucx", SearchOption.AllDirectories)
            .ToList();
        ...
        foreach (string file in componentFiles)
        {
            ...
            if (name != null && !name.StartsWith("Meadow/", StringComparison.Ordinal))
            {
    ...
    public void AllBehaviorFiles_ShouldBeSharedAndUnnamespaced()
    {
        ...
        // folder is normally empty. This test guards that a future Meadow-only behavior
        // (if one is ever added here instead of shared) stays unnamespaced.
        string behaviorsDir = Path.Combine(FindRepoRoot(),
            "Tools", "Gum.ProjectServices", "Templates", "FormsThemes", "Meadow", "Behaviors");
    ...
    [Fact]
    public void StylesPalette_ShouldExactlyMatchCodeOnlyMeadowColors()
    {
        // Components/Meadow/Styles.gucx exists so tool content can reference one set of
        // color/text tokens instead of hardcoding values. Its swatch names must match
        // Themes/Gum.Themes.Meadow.MonoGame/MeadowStyling.cs's MeadowColors/MeadowText properties
        // exactly (restricted to the subset any *Visual.cs actually references), and carry no
        // swatch beyond that - otherwise the tool content silently drifts from the theme it's
        // supposed to mirror. Excluded by design (computed/alias properties with no distinct
        // settable value of their own): TextPrimary/TextMuted/Primary/Accent (guardrail aliases
        // onto TealDark/Muted/Cream2/Blue), HoverRow/HoverOption (alias onto PeachLight),
        // SelectedRow/SelectedRowText (aliases onto Sage/TealDark), and the unused Peach/SageLight/
        // SelectedRowInset (no *Visual.cs references them).
        string[] expectedColorSwatches =
        {
            "Blue", "BlueDark", "BlueFocusRing", "BlueHover",
            "Coral", "CoralDark", "Cream", "Cream2",
            "Disabled", "DisabledInk", "DisabledSliderFill",
            "Muted", "PeachDark", "PeachLight",
            "Sage", "SageDark", "SageFocusRing",
            "Teal", "TealDark", "ThumbShadow", "White", "WindowShadow"
        };
        string[] expectedTextStyles = { "Body", "Normal", "Strong" };

        GumProjectSave project = LoadMeadow();
        ComponentSave styles = project.Components.Single(c => c.Name == "Meadow/Styles");
