...
public class VariablesTabTests
{
    ...
    [AvaloniaFact]
    public void IconRegistry_ResolvesEveryToggleOptionsIcon()
    {
        GumIconRegistry icons = Services.GetRequiredService<GumIconRegistry>();
        ...
        {
            icons.CreateIcon(option.GumIconName!).ShouldNotBeNull(option.GumIconName);
        }
        icons.CreateIcon("NoSuchIcon").ShouldBeNull();
    }
