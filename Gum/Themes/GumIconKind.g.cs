// <auto-generated/>
using System;
using System.ComponentModel;
using System.Globalization;
using System.Linq;

namespace Gum.Themes;

[TypeConverter(typeof(GumIconKindConverter))]
public enum GumIconKind
{
    None = 0,
    ChildrenLayoutAutoGridHorizontal, // ChildrenLayoutAutoGridHorizontal
    ChildrenLayoutAutoGridVertical, // ChildrenLayoutAutoGridVertical
    ChildrenLayoutLeftToRightStack, // ChildrenLayoutLeftToRightStack
    ChildrenLayoutRegular, // ChildrenLayoutRegular
    ChildrenLayoutTopToBottomStack, // ChildrenLayoutTopToBottomStack
    HeightUnitsAbsolute, // HeightUnitsAbsolute
    HeightUnitsAbsoluteMultipliedByFontScale, // HeightUnitsAbsoluteMultipliedByFontScale
    HeightUnitsMaintainFileAspectRatio, // HeightUnitsMaintainFileAspectRatio
    HeightUnitsPercentageOfFileHeight, // HeightUnitsPercentageOfFileHeight
    HeightUnitsPercentageOfParent, // HeightUnitsPercentageOfParent
    HeightUnitsPercentageOfWidth, // HeightUnitsPercentageOfWidth
    HeightUnitsRatioOfParent, // HeightUnitsRatioOfParent
    HeightUnitsRelativeToChildren, // HeightUnitsRelativeToChildren
    HeightUnitsRelativeToParent, // HeightUnitsRelativeToParent
    TextOverflowHorizontalEllipsisLetter, // TextOverflowHorizontalEllipsisLetter
    TextOverflowHorizontalTruncateWord, // TextOverflowHorizontalTruncateWord
    TextOverflowVerticalSpill, // TextOverflowVerticalSpill
    TextOverflowVerticalTruncateLine, // TextOverflowVerticalTruncateLine
    WidthUnitsAbsolute, // WidthUnitsAbsolute
    WidthUnitsAbsoluteMultipliedByFontScale, // WidthUnitsAbsoluteMultipliedByFontScale
    WidthUnitsMaintainFileAspectRatio, // WidthUnitsMaintainFileAspectRatio
    WidthUnitsPercentageOfFileWidth, // WidthUnitsPercentageOfFileWidth
    WidthUnitsPercentageOfHeight, // WidthUnitsPercentageOfHeight
    WidthUnitsPercentageOfParent, // WidthUnitsPercentageOfParent
    WidthUnitsRatioOfParent, // WidthUnitsRatioOfParent
    WidthUnitsRelativeToChildren, // WidthUnitsRelativeToChildren
    WidthUnitsRelativeToParent, // WidthUnitsRelativeToParent
    XOriginCenter, // XOriginCenter
    XOriginEnd, // XOriginEnd
    XOriginStart, // XOriginStart
    XUnitsCenter, // XUnitsCenter
    XUnitsLeft, // XUnitsLeft
    XUnitsPercentageParent, // XUnitsPercentageParent
    XUnitsRight, // XUnitsRight
    YOriginBaseline, // YOriginBaseline
    YOriginCenter, // YOriginCenter
    YOriginEnd, // YOriginEnd
    YOriginStart, // YOriginStart
    YUnitsBaseline, // YUnitsBaseline
    YUnitsBottom, // YUnitsBottom
    YUnitsCenter, // YUnitsCenter
    YUnitsPercentageParent, // YUnitsPercentageParent
    YUnitsTop, // YUnitsTop
}

public static class GumIconKindMap
{
    private static readonly string[] _keys = new string[] {
        null, // None
        "ChildrenLayoutAutoGridHorizontal",
        "ChildrenLayoutAutoGridVertical",
        "ChildrenLayoutLeftToRightStack",
        "ChildrenLayoutRegular",
        "ChildrenLayoutTopToBottomStack",
        "HeightUnitsAbsolute",
        "HeightUnitsAbsoluteMultipliedByFontScale",
        "HeightUnitsMaintainFileAspectRatio",
        "HeightUnitsPercentageOfFileHeight",
        "HeightUnitsPercentageOfParent",
        "HeightUnitsPercentageOfWidth",
        "HeightUnitsRatioOfParent",
        "HeightUnitsRelativeToChildren",
        "HeightUnitsRelativeToParent",
        "TextOverflowHorizontalEllipsisLetter",
        "TextOverflowHorizontalTruncateWord",
        "TextOverflowVerticalSpill",
        "TextOverflowVerticalTruncateLine",
        "WidthUnitsAbsolute",
        "WidthUnitsAbsoluteMultipliedByFontScale",
        "WidthUnitsMaintainFileAspectRatio",
        "WidthUnitsPercentageOfFileWidth",
        "WidthUnitsPercentageOfHeight",
        "WidthUnitsPercentageOfParent",
        "WidthUnitsRatioOfParent",
        "WidthUnitsRelativeToChildren",
        "WidthUnitsRelativeToParent",
        "XOriginCenter",
        "XOriginEnd",
        "XOriginStart",
        "XUnitsCenter",
        "XUnitsLeft",
        "XUnitsPercentageParent",
        "XUnitsRight",
        "YOriginBaseline",
        "YOriginCenter",
        "YOriginEnd",
        "YOriginStart",
        "YUnitsBaseline",
        "YUnitsBottom",
        "YUnitsCenter",
        "YUnitsPercentageParent",
        "YUnitsTop",
    };

    public static string? GetResourceKey(GumIconKind kind) =>
        (int)kind >= 0 && (int)kind < _keys.Length ? _keys[(int)kind] : null;
}

// Lets you write Icon="folder-star" or Icon="FolderStar" in XAML
public sealed class GumIconKindConverter : TypeConverter
{
    public override bool CanConvertFrom(ITypeDescriptorContext? ctx, Type t) =>
        t == typeof(string) || base.CanConvertFrom(ctx, t);
    public override object? ConvertFrom(ITypeDescriptorContext? ctx, CultureInfo? culture, object value)
    {
        if (value is string s)
        {
            var normalized = ToEnumTokenForConverter(s);
            if (Enum.TryParse<GumIconKind>(normalized, ignoreCase: false, out var result))
                return result!;
        }
        return base.ConvertFrom(ctx, culture, value);
    }
    private static string ToEnumTokenForConverter(string name)
    {
        var parts = System.Text.RegularExpressions.Regex.Split(name, @"[^A-Za-z0-9]+").Where(p => p.Length > 0).Select(p => char.ToUpperInvariant(p[0]) + p.Substring(1));
        var token = string.Concat(parts);
        if (string.IsNullOrEmpty(token)) token = "Icon";
        if (char.IsDigit(token[0])) token = "_" + token;
        return token;
    }
}
