...
* **Always create interfaces for services, managers, and helpers**: When creating any new service, manager, or helper class that will be registered in dependency injection (Builder.cs), MUST create a matching interface (I[ClassName]). Register the interface in Builder.cs (e.g., `services.AddSingleton<IMyService, MyService>()`), and use the interface type in all constructor parameters, fields, and references. This ensures loose coupling, testability, and consistency with existing Gum patterns.
