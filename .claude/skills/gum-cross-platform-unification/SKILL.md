...
## Loader Registrations Must Instantiate the Most-Derived Shim

When a runtime type is moved to `Gum.GueDeriving` and the old name is kept as an `[Obsolete]` **derived** shim subclass (`MonoGameGum.GueDeriving.RoundedRectangleRuntime : Gum.GueDeriving.RoundedRectangleRuntime`), **every** loader registration that produces it (`ElementSaveExtensions.RegisterGueInstantiation`) must instantiate the **shim**, not the new base. Already-generated user code downcasts the loaded instance to the *old* shim namespace (`... as MonoGameGum.GueDeriving.RoundedRectangleRuntime`); `base as derived` is `null` → `NullReferenceException` on first use, with no compile error and no regen fix (current codegen emits the new namespace, so regenerating produces byte-identical output). The shim is-a base, so instantiating the shim satisfies the old shim cast **and** any newer base cast. Qualify the `new` with the shim namespace and wrap in `#pragma warning disable CS0618`.

These registration sites are **separate and easy to miss one**: `RenderingLibrary.SystemManagers` (XNA standard types), `AposShapeRuntime` (Apos shapes — `Gum.GueDeriving` namespace, so the unqualified name resolves to the *base*), and SkiaGum's `SystemManagers` (`using Gum.GueDeriving;`, same trap). Convert all of them together. Exception: a type that exists only in the new namespace with **no** shim (e.g. Skia's `RectangleRuntime`) stays the base. (Issue #3380 — broke FlatRedBall2/Solitaire after only the XNA standard types had been converted, leaving the Apos shapes and all of Skia instantiating the base.)
