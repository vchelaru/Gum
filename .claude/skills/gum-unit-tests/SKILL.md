...
**A golden test that renders text must not depend on a system font family** (e.g. `FontName = "Arial"`) — glyph shapes come from whatever font is actually installed on the machine, which differs across CI runners (the macOS Actions image has no Arial and silently substitutes a different typeface, blowing the pixel tolerance; confirmed on #3692's `TextCustomizationGoldenImageTests`, which passed locally on Windows and failed in CI on macOS). Instead load a bundled TTF (`GoldenImages/Fonts/`, wired via the same `CopyToOutputDirectory` pattern) directly with `SKTypeface.FromFile`, and install it for the test via a `Topten.RichTextKit.FontMapper` subclass assigned to the static `FontMapper.Default` (restore the original in `Dispose`) — RichTextKit resolves any `TextBlock` that doesn't set an explicit `FontMapper` through that static default, so this makes every glyph render from the bundled font regardless of what's installed on the host. See `TextCustomizationGoldenImageTests.FixedFontMapper` for the pattern.

**A golden test must not feed `Math.Sin`/`MathF.Sin`/`Cos`-derived values into a pixel-position or color it then compares byte-for-byte.** Those call into the OS's math library (ucrt/libSystem/glibc), which are not guaranteed bit-identical cross-platform; a last-bit difference nudges a glyph by a sub-pixel amount and flips an antialiased edge pixel, failing the comparison even with an identical bundled font (this is what still broke `TextCustomizationGoldenImageTests` after the font fix above — same test, same platform pairing). Use a fixed lookup table of exact integers/bytes for anything a golden test's callback feeds into position or color; keep the real trig-based version (which only needs to look right, not byte-match a baseline) in the samples.
