...
- **A bubbling-only subscriber may not count as "over."** The default `VisualOverBehavior` is `OnlyIfEventsAreNotNullAndHasEventsIsTrue`, which decides an element is "over" only if one of a hard-coded list of events (in the `shouldTreatAsIsOver` block) is non-null. A new bubbling event must be **added to that list**, or an element subscribing to *only* that event is skipped and never fires. `MouseWheelScroll` and `ClickBubbling` are in the list; `RollOverBubbling` notably is not.
- **Participation is multi-gated.** Beyond `HasEvents`, an element's involvement depends on `ExposeChildrenEvents`, `IsEnabledRecursively`, and `VisualOverBehavior`. All four can independently keep an event from firing.
