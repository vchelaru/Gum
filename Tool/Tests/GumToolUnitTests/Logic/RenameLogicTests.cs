...
public class RenameLogicTests : BaseTestClass
{
    ...
    #region Behavior state rename

    [Fact]
    public void GetChangesForRenamedState_BehaviorContainer_MatchingVariable_IsDetected()
    {
        // Behavior has a "Hover" category with state "Hovered".
        // Button implements the behavior and mirrors the category/state on itself;
        // Button's Default state directly sets HoverState = "Hovered".
        // Screen has instance of Button with variable "myButton.HoverState = Hovered".
        // GetChangesForRenamedState should detect both variables.
        BehaviorSave behavior = new BehaviorSave { Name = "Clickable" };
        StateSaveCategory hoverCategory = new StateSaveCategory { Name = "Hover" };
        StateSave hoveredState = new StateSave { Name = "Hovered" };
        hoverCategory.States.Add(hoveredState);
        behavior.Categories.Add(hoverCategory);
        ...
        VariableSave buttonStateVar = new VariableSave { Name = "HoverState", Value = "Hovered" };
        buttonDefault.Variables.Add(buttonStateVar);
        button.States.Add(buttonDefault);
        StateSaveCategory buttonHoverCategory = new StateSaveCategory { Name = "Hover" };
        StateSave buttonHoveredState = new StateSave { Name = "Hovered", ParentContainer = button };
        buttonHoverCategory.States.Add(buttonHoveredState);
        button.Categories.Add(buttonHoverCategory);
        button.Behaviors.Add(new ElementBehaviorReference { BehaviorName = "Clickable" });
        _project.Components.Add(button);

        ScreenSave screen = new ScreenSave { Name = "GameScreen" };
        ...
        screen.Instances.Add(instance);
        VariableSave screenStateVar = new VariableSave { Name = "myButton.HoverState", Value = "Hovered" };
        screenDefault.Variables.Add(screenStateVar);
        _project.Screens.Add(screen);

        StateRenameChanges changes = _renameLogic.GetChangesForRenamedState(hoveredState, "Hovered", behavior, hoverCategory);

        changes.VariablesToUpdate.Count.ShouldBe(2);
        changes.VariablesToUpdate.ShouldContain(item => item.Container == button && item.Variable == buttonStateVar);
        changes.VariablesToUpdate.ShouldContain(item => item.Container == screen && item.Variable == screenStateVar);

        changes.StatesToRename.Count.ShouldBe(1);
        changes.StatesToRename[0].Element.ShouldBe(button);
        changes.StatesToRename[0].State.ShouldBe(buttonHoveredState);
    }

    [Fact]
    public void GetChangesForRenamedState_BehaviorContainer_NoImplementingComponent_ReturnsEmpty()
    {
        // Behavior exists but no component implements it → no changes.
        var behavior = new BehaviorSave { Name = "Clickable" };
        var hoverCategory = new StateSaveCategory { Name = "Hover" };
        var hoveredState = new StateSave { Name = "Hovered" };
        hoverCategory.States.Add(hoveredState);
        behavior.Categories.Add(hoverCategory);

        var screen = new ScreenSave { Name = "GameScreen" };
        screen.States.Add(new StateSave { Name = "Default", ParentContainer = screen });
        _project.Screens.Add(screen);

        var changes = _renameLogic.GetChangesForRenamedState(hoveredState, "Hovered", behavior, hoverCategory);

        changes.VariablesToUpdate.ShouldBeEmpty();
    }

    [Fact]
    public void GetChangesForRenamedState_BehaviorContainer_MultipleImplementingComponents_AllDetected()
    {
        // Two components implement the same behavior.
        // Each appears in a different screen.
        // Both referencing variables should be detected.
        var behavior = new BehaviorSave { Name = "Clickable" };
        var hoverCategory = new StateSaveCategory { Name = "Hover" };
        var hoveredState = new StateSave { Name = "Hovered" };
        hoverCategory.States.Add(hoveredState);
        ...
        var toggle = new ComponentSave { Name = "Toggle" };
        toggle.States.Add(new StateSave { Name = "Default", ParentContainer = toggle });
        toggle.Behaviors.Add(new ElementBehaviorReference { BehaviorName = "Clickable" });
        _project.Components.Add(toggle);

        var screen1 = new ScreenSave { Name = "Screen1" };
        var screen1Default = new StateSave { Name = "Default", ParentContainer = screen1 };
        screen1.States.Add(screen1Default);
        var inst1 = new InstanceSave { Name = "myButton", BaseType = "Button", ParentContainer = screen1 };
        screen1.Instances.Add(inst1);
        var stateVar1 = new VariableSave { Name = "myButton.HoverState", Value = "Hovered" };
        screen1Default.Variables.Add(stateVar1);
        _project.Screens.Add(screen1);

        var screen2 = new ScreenSave { Name = "Screen2" };
        var screen2Default = new StateSave { Name = "Default", ParentContainer = screen2 };
        screen2.States.Add(screen2Default);
        var inst2 = new InstanceSave { Name = "myToggle", BaseType = "Toggle", ParentContainer = screen2 };
        screen2.Instances.Add(inst2);
        var stateVar2 = new VariableSave { Name = "myToggle.HoverState", Value = "Hovered" };
        screen2Default.Variables.Add(stateVar2);
        _project.Screens.Add(screen2);

        var changes = _renameLogic.GetChangesForRenamedState(hoveredState, "Hovered", behavior, hoverCategory);

        changes.VariablesToUpdate.Count.ShouldBe(2);
        changes.VariablesToUpdate.ShouldContain(item => item.Variable == stateVar1);
        changes.VariablesToUpdate.ShouldContain(item => item.Variable == stateVar2);
    }

    [Fact]
    public void GetChangesForRenamedState_BehaviorContainer_ImplementingComponentOwnStateVariable_IsDetected()
    {
        // Behavior "Clickable" has "Hover" category with state "Hovered".
        // Component "Button" implements Clickable.
        // Button itself (not as an instance in some other element) has a state in a
        // non-behavior category whose variable directly sets HoverState = "Hovered"
        // (no instance-name prefix).
        // GetChangesForRenamedState should detect that variable.
        BehaviorSave behavior = new BehaviorSave { Name = "Clickable" };
        StateSaveCategory hoverCategory = new StateSaveCategory { Name = "Hover" };
        StateSave hoveredState = new StateSave { Name = "Hovered" };
        hoverCategory.States.Add(hoveredState);
        ...
        // A "Visibility" category on Button; its "Visible" state explicitly sets HoverState.
        StateSaveCategory visibilityCategory = new StateSaveCategory { Name = "Visibility" };
        StateSave visibleState = new StateSave { Name = "Visible", ParentContainer = button };
        VariableSave stateVar = new VariableSave { Name = "HoverState", Value = "Hovered" };
        visibleState.Variables.Add(stateVar);
        visibilityCategory.States.Add(visibleState);
        ...
        changes.VariablesToUpdate[0].Container.ShouldBe(button);
        changes.VariablesToUpdate[0].Variable.ShouldBe(stateVar);
    }

    [Fact]
    public void GetChangesForRenamedState_BehaviorContainer_DerivedComponentOwnStateVariable_IsDetected()
    {
        // Behavior "Clickable" has "Hover" category with state "Hovered".
        // Component "Button" explicitly implements Clickable.
        // Component "FancyButton" derives from Button (BaseType = "Button") but does NOT
        // explicitly list the Clickable behavior — it inherits it through the base type.
        // FancyButton has a state that directly sets HoverState = "Hovered" on itself.
        // GetChangesForRenamedState should detect that variable because FancyButton
        // transitively uses the behavior.
        BehaviorSave behavior = new BehaviorSave { Name = "Clickable" };
        StateSaveCategory hoverCategory = new StateSaveCategory { Name = "Hover" };
        StateSave hoveredState = new StateSave { Name = "Hovered" };
        hoverCategory.States.Add(hoveredState);
        ...
        // FancyButton does NOT explicitly add the Clickable behavior reference.
        StateSaveCategory visibilityCategory = new StateSaveCategory { Name = "Visibility" };
        StateSave visibleState = new StateSave { Name = "Visible", ParentContainer = fancyButton };
        VariableSave stateVar = new VariableSave { Name = "HoverState", Value = "Hovered" };
        visibleState.Variables.Add(stateVar);
        visibilityCategory.States.Add(visibleState);
        fancyButton.Categories.Add(visibilityCategory);
        _project.Components.Add(fancyButton);

        StateRenameChanges changes = _renameLogic.GetChangesForRenamedState(hoveredState, "Hovered", behavior, hoverCategory);

        changes.VariablesToUpdate.Count.ShouldBe(1);
        changes.VariablesToUpdate[0].Container.ShouldBe(fancyButton);
        changes.VariablesToUpdate[0].Variable.ShouldBe(stateVar);
    }
    ...
    [Fact]
    public void ApplyStateRenameChanges_RenamesMatchingStateInComponent()
    {
        // A component has a state "Hovered" that was listed in StatesToRename.
        // After Apply, that state's Name should match the renamed state's new name.
        ComponentSave button = new ComponentSave { Name = "Button" };
        StateSaveCategory hoverCategory = new StateSaveCategory { Name = "Hover" };
        StateSave buttonHoveredState = new StateSave { Name = "Hovered", ParentContainer = button };
        hoverCategory.States.Add(buttonHoveredState);
        button.Categories.Add(hoverCategory);
        _project.Components.Add(button);

        StateSave renamedState = new StateSave { Name = "Highlighted" };

        StateRenameChanges changes = new StateRenameChanges();
        changes.StatesToRename.Add((button, buttonHoveredState));

        _renameLogic.ApplyStateRenameChanges(changes, renamedState);

        buttonHoveredState.Name.ShouldBe("Highlighted");
    }
    ...
    [Fact]
    public void GetChangesForRenamedCategory_BehaviorOwner_MatchingVariable_IsDetected()
    {
        // Behavior "Clickable" has category "Hover".
        // Component "Button" implements the behavior.
        // Screen has instance of Button with a variable of Type "Hover".
        // GetChangesForRenamedCategory should detect this variable.
        var behavior = new BehaviorSave { Name = "Clickable" };
        var hoverCategory = new StateSaveCategory { Name = "Hover" };
        behavior.Categories.Add(hoverCategory);
        ...
        var instance = new InstanceSave { Name = "myButton", BaseType = "Button", ParentContainer = screen };
        screen.Instances.Add(instance);
        var stateVar = new VariableSave { Name = "myButton.HoverState", Type = "Hover" };
        screenDefault.Variables.Add(stateVar);
        _project.Screens.Add(screen);

        var changes = _renameLogic.GetChangesForRenamedCategory(behavior, hoverCategory, "Hover");
    ...
    [Fact]
    public void GetChangesForRenamedCategory_BehaviorOwner_NoImplementingComponent_ReturnsEmpty()
    {
        // Behavior exists but no component implements it → no changes.
        var behavior = new BehaviorSave { Name = "Clickable" };
        var hoverCategory = new StateSaveCategory { Name = "Hover" };
    ...
    [Fact]
    public void GetChangesForRenamedCategory_BehaviorOwner_ComponentInheritingFromImplementor_IsDetected()
    {
        // Behavior "Clickable" has category "Hover".
        // Component "Button" implements the behavior.
        // Component "ButtonChild" inherits from Button.
        // Screen has instance of ButtonChild with a variable of Type "Hover".
        // Should detect via inheritance chain.
        var behavior = new BehaviorSave { Name = "Clickable" };
        var hoverCategory = new StateSaveCategory { Name = "Hover" };
        behavior.Categories.Add(hoverCategory);
    ...
    [Fact]
    public void ApplyCategoryRenameChanges_RenamesMatchingCategoryInComponent()
    {
        // A component has a "Hover" category listed in CategoriesToRename.
        // After Apply, that category's Name should be updated to the new name.
        BehaviorSave behavior = new BehaviorSave { Name = "Clickable" };
        StateSaveCategory behaviorHoverCategory = new StateSaveCategory { Name = "Hover" };
        behavior.Categories.Add(behaviorHoverCategory);
        ...
        CategoryRenameChanges changes = new CategoryRenameChanges();
        changes.CategoriesToRename.Add((button, buttonHoverCategory));

        _renameLogic.ApplyCategoryRenameChanges(changes, behavior, behaviorHoverCategory, "Hover", "Focus");

        buttonHoverCategory.Name.ShouldBe("Focus");
    }
