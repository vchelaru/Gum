...
/// <summary>
/// The editor tab, complete except for what a UI framework must supply: the canvas control
/// (<see cref="CreateCanvas"/>), the tab's layout with its toolbar and scroll bars
/// (<see cref="BuildEditorTab"/>), the right-click menu (<see cref="ShowContextMenu"/>), and the
/// optional shader resolver. Each head derives an exported plugin from this and implements those.
/// The head also owns its drag and drop reader and feeds the neutral
/// <see cref="WireframeDropPayload"/> to <see cref="DecideWireframeDropAccepted"/> and
...
#pragma warning restore CA1001 // Types that own disposable fields should be disposable
{
    ...
    /// <summary>Creates the head's canvas control and returns the neutral core it hosts.</summary>
    protected abstract WireframeCanvasCore CreateCanvas(IDialogService dialogService, IOutputManager outputManager, IPluginManager pluginManager);

    /// <summary>
    /// Lays out the editor tab (toolbar over <paramref name="editorViewModel"/>, grid-snap warning,
    /// canvas, scroll bars), hands the scroll bars to <paramref name="scrollbarService"/>, and adds
    /// the tab through the tab manager.
    /// </summary>
    protected abstract void BuildEditorTab(EditorViewModel editorViewModel, ScrollbarService scrollbarService);

    /// <summary>
    /// The resolver that compiles a render-target Container's SourceShaderFile into an Effect for
    /// the preview, or null when the head has none (shaded containers then render unshaded).
    /// </summary>
    protected abstract Func<string, object?>? CreateRenderTargetShaderResolver();

    /// <summary>Whether the head's canvas context menu is showing.</summary>
    protected abstract bool IsContextMenuOpen { get; }

    /// <summary>Shows <paramref name="items"/> as the canvas context menu at the pointer.</summary>
    protected abstract void ShowContextMenu(IReadOnlyList<ContextMenuItemViewModel> items);

    /// <summary>The app-wide base font size changed; heads with sized toolbar buttons update them.</summary>
    protected virtual void OnUiBaseFontSizeChanged(double size)
    {
    }

    /// <summary>The head calls this when its canvas control changes size.</summary>
    protected void OnCanvasResized() => _pluginManager.HandleWireframeResized();

    /// <summary>The head calls this on a right click over the canvas.</summary>
    protected void ShowCanvasContextMenu()
    {
        _editingManager.OnRightClick();
        if (_editingManager.ContextMenuItems.Count > 0)
        {
            ShowContextMenu(_editingManager.ContextMenuItems);
        }
    }

    #endregion

    protected EditorTabPluginBase(
    ...
    public override void StartUp()
    {
        ...
        _renderTargetShaderResolver = CreateRenderTargetShaderResolver();
    ...
    {
        object? effect = _renderTargetShaderResolver?.Invoke(absolutePath);
        if (effect != null)
        {
    ...
    private void HandleXnaInitialized()
    {
        ...
        _editingManager.Initialize(() => IsContextMenuOpen);

        _backgroundManager.Initialize(_canvas.SystemManagers);

        _scrollbarService.HandleXnaInitialized();


        _canvas.CameraChanged += () =>
        {
        ...
        _canvas.DesiredFramesPerSecond = frameRate;
    ...
    /// <summary>
    /// Whether a drag carrying <paramref name="payload"/> may drop on the canvas. With
    /// <paramref name="reportBlockedReason"/> a rejection is explained in the Output tab; a head
    /// passes false for the per-move re-evaluation so the reason prints once per drag.
    /// </summary>
    protected internal bool DecideWireframeDropAccepted(WireframeDropPayload payload, bool reportBlockedReason)
    {
    ...
    protected internal void HandleWireframeDrop(WireframeDropPayload payload)
    {
    ...
    {
        _canvas = CreateCanvas(_dialogService, _outputManager, _pluginManager);
        _canvas.DesiredFramesPerSecond = 30F;
        ...
        BuildEditorTab(_editorViewModel, _scrollbarService);
    }
    ...
    {
        _canvas.BackgroundColor = ToXna(settings.CheckerA);
