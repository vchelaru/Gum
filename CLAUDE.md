...
**Tool work defaults to the Avalonia head (`Tool/Gum.Avalonia`), not WPF (`Gum/`, `Gum.Wpf.sln`).** Per ADR-0017 (`Direction/decisions/0017-commit-to-avalonia-full-cutover.md`), WPF is frozen: no new tool features or refactors there. On an ambiguous task ("implement this feature," "fix this bug" with no head named), assume Avalonia. Touch WPF only when the user explicitly names it, or for a fix that must ship on the last WPF release before cutover. Full removal of WPF/WinForms is tracked as its own future PR in `Direction/avalonia-migration/phase-120-cutover.md` — until that lands, both heads still exist in the repo, but only one gets new work.
...
* **Tool work — default to the Avalonia head** (see Project Direction above); use `Gum.slnx` (the Avalonia tool with its plugins). Only build via `Gum.Wpf.sln` (WPF tool, no CLI) or `GumFull.sln` (WPF tool plus CLI) when the task explicitly targets WPF. The release ships the Avalonia head only. Anything under `Tool/`, `Gum/`, plugins, or tool-side code **must** be built via the solution — plugin projects use `$(SolutionDir)` in post-build scripts, which is undefined when building a `.csproj` directly, so building individual tool csprojs silently breaks plugin output.
...
* **Avalonia head (`Tool/Gum.Avalonia`)** — plain `net10.0`, no `$(SolutionDir)` post-build, no WPF/WinForms anywhere in its reference graph (`Direction/avalonia-migration/`). Build with `dotnet build Tool/Gum.Avalonia/Gum.Avalonia.csproj`; test with `dotnet test Tests/Gum.Avalonia.Tests` (headless, runs on any OS). It composes `AddGumCore()` from `Gum.Presentation` plus `AddGumAvalonia()`; a service that only the WPF head implements must be added to `HeadProvidedContracts` and given an Avalonia implementation, or the head fails to compose. It may be launched unattended for verification: `dotnet run --project Tool/Gum.Avalonia -- --exit-after 8 --screenshot <path>.png` (this is the one GUI launch that is allowed; it exits by itself). Add a project path to open it, `--select Element[#Instance]` to select something once it loads, `--theme light|dark` to show a theme variant without saving it, and **always** `--user-data <temp folder>` so the run's last-project/recent-list writes stay out of the user's real settings (`%APPDATA%\Gum.Avalonia`). Opening a project can re-save it (legacy version upgrade), so point it at a copy, never at `Samples/`. Comparing the two heads side by side (screenshot driver, UI Automation probes, Linux run recipe) is `Tools/ParityShots/README.md`.
