...
* **Tool work (`GumFull.sln`)** — anything under `Tool/`, `Gum/`, plugins, or tool-side code **must** be built via the solution. (`Gum.slnx` is the Avalonia tool with its plugins; `Gum.Wpf.sln` is the WPF tool without the CLI; `GumFull.sln` is the WPF tool plus CLI. The release ships the Avalonia head only.) Plugin projects use `$(SolutionDir)` in post-build scripts, which is undefined when building a `.csproj` directly, so building individual tool csprojs silently breaks plugin output.
